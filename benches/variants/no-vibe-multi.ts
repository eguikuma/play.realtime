...
/**
 * 直近のシナリオ実行 (`report/*.json`) を `variants/no-vibe-multi/` にコピーして候補 2 改善後の数値として保管する
 * baseline と throttle と no-hexists と room-cache と並べて `RedisVibeRepository.save` `delete` の MULTI/EXEC 撤廃による vibe namespace 削減を `report.ts` で並べる
 * 既存の no-vibe-multi は上書きする想定、複数世代を取りたいなら本スクリプトを複製して別名にする
 */

const root = resolve(import.meta.dirname, "..");
const sourceDirectory = resolve(root, "report");
const snapshotDirectory = resolve(root, "variants", "no-vibe-multi");
