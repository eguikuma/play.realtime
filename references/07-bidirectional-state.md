...
WS の選定理由、招待や応答のような双方向のやり取りが要るときに WS を選ぶ判断

dispatch 決定論性、招待タイマーの TTL 切れも 多 instance では 1 instance だけが fire を担当する
...
招待タイマーは dispatch 決定論性 と同じ仕組みに乗る
...
- 招待タイマー（backend の infrastructure 層 timer、dispatch と同じ owner side key 方式を採用）
