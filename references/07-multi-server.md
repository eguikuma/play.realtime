...
```
サーバ a ── SETNX done:room-X "1" ── Redis    成功
サーバ b ── SETNX done:room-X "1" ── Redis    失敗、すでに書かれている
```
...
room:grace:room-X         TTL 30 秒で expire
room:grace:owner:room-X   "instance-a"   TTL 5 分の余裕
```
...
```
サーバ a ── GET room:grace:owner:room-X ── "instance-a"
サーバ b ── GET room:grace:owner:room-X ── "instance-a"
```
...
```
usecase 層 ── PubSub.publish ── ┌─ in-memory pub/sub   1 台用
                                  └─ Redis pub/sub      複数台用
```
