...
Redis pubsub、TTL key の挙動と keyspace notification の届き方

grace timer、その制御を 多 instance 環境で誰が握るかという問題意識
...
同じ key に対する re schedule で、所有権は最後に書いた instance に移る

これは grace timer の延長や 移し替えの自然な意味と一致する
...
これが 「敗者は再試行しない」 性質と噛み合わずに機能不全になる
