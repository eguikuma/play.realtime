...
/**
 * API サーバの Origin
 * ビルド時に `NEXT_PUBLIC_BACKEND_ORIGIN` が与えられればその値、未設定ならローカル開発の既定値を使い、不正な URL なら起動時点で throw する
 */
export const origin = z
  .url()
  .parse(process.env.NEXT_PUBLIC_BACKEND_ORIGIN ?? "http://localhost:4000");
