...
/**
 * 廊下トーク永続化の port 型
 * 招待と通話の 2 種類の集約を扱い、メンバー単位とルーム単位の探索、発信側と着信側の区別など用途別にメソッドを分けている
 */
export type HallwayRepository = {
  ...
  /**
   * 指定 ID の招待を削除する
   * `Accept` / `Decline` / `Cancel` / `expired` のどのルートでも最終的に呼ばれる
   * */
  ...
  /**
   * 指定 ID の通話を削除する
   * `Leave` と `disconnect` のどちらのルートでも最終的に呼ばれる
   * */
  deleteCall: (id: CallId) => Promise<void>;
  /**
   * 指定ルームに属する全招待と全通話を削除する
   * ルーム閉鎖時のクリーンアップで使う
   * */
