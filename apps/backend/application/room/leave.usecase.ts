 ...
 * 受信側 (両 Hub の `closeByMember`) が自インスタンスの保持接続だけ強制クローズし、close ハンドラ経由で `RoomPresence.detach` と機能別 broadcaster を踏ませる
 *
 * `Room.members` は入場名簿として時間的に単調増加する集合のため、ここでは一切更新しない
 * 在室実態は Vibe aggregate が connection 単位で管理しており、明示退出は Vibe 側の detach と Hub の close だけで完結する
 */
@Injectable()
